...
export function AddTimeSlotForm({ dayOfWeek }: { dayOfWeek: number }) {
  ...
  return (
    <form ref={formRef} action={formAction} className="flex items-center gap-2 text-sm">
      ...
      <input type="time" name="start_time" required className="w-24 rounded border border-neutral-700 px-1 py-0.5" />
      <span className="text-neutral-600">–</span>
      <input type="time" name="end_time" required className="w-24 rounded border border-neutral-700 px-1 py-0.5" />
      <button
        type="submit"
        disabled={pending}
        className="rounded border border-neutral-700 px-2 py-0.5 text-sm disabled:opacity-50"
      >
